...
export default function TabBar() {
  const { openTabs, activeFile, setActiveFile, closeTab, unsavedFiles } = useStore();
  ...
  return (
    ...
    >
      {openTabs.map(tab => {
        ...
        const isUnsaved = unsavedFiles.has(tab);
        return (
          <div
            key={tab}
          ...
          >
            {icon && <span className="text-[9px] font-bold flex-shrink-0" style={{ color }}>{icon}</span>}
            <span
            ...
            {/* Unsaved dot in tab */}
            {isUnsaved && (
              <span style={{ color: "#f9e2af", fontSize: 10, lineHeight: 1 }} title="Sin guardar">●</span>
            )}
            <button
              onClick={e => { e.stopPropagation(); closeTab(tab); }}
              className="ml-auto opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity flex-shrink-0"
              style={{ color: "#f87171" }}
            >
              <X size={10} />
            </button>
