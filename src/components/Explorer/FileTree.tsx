...
export default function FileExplorer() {
  ...
  // Auto-load persisted project on mount
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full" style={{background:"#0f0f1a"}}>
      <div className="flex items-center justify-between px-3 py-1.5 flex-shrink-0" style={{borderBottom:"1px solid #1e1e35"}}>
        {folderName
          ? <span className="text-[10px] font-mono truncate max-w-[140px]" style={{color:"#4a4a6a"}}>{folderName}</span>
          : <span className="text-[10px]" style={{color:"#2e2e4a"}}>Sin carpeta</span>
        }
        <button onClick={openFolder} title="Abrir carpeta" style={{color:"#4a4a6a"}} className="hover:opacity-100 opacity-60 transition-opacity p-0.5 rounded">
          <FolderPlus size={12}/>
        </button>
      </div>
